...
from func.nn.activation import HyperbolicTangentSigmoid as TANH
...
def main(P,mate,mutate):
    """Run this experiment"""
    training_ints = initialize_instances('training.csv')
    testing_ints = initialize_instances('test.csv')
    validation_ints = initialize_instances('validation.csv')
    ...
    classification_network = factory.createClassificationNetwork([INPUT_LAYER, HIDDEN_LAYER1, OUTPUT_LAYER],tanh)
...
if __name__ == "__main__":
    for p in [100]:
        for mate in [10, 30, 50]:
            for mutate in [10, 30, 50]:
                main(p,mate,mutate)